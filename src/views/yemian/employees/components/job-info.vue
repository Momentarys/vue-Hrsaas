<template>
  <div class="job-info">
    ...
    <el-form label-width="220px">
      ...
      <!-- 从业信息 -->
      <el-form-item>
        <el-button type="primary" @click="saveEmployeesJob">保存更新</el-button>
...
import { gitEmployeesJobInof, saveEmployeesJobInof } from '@/api/employees'
export default {
  data() {
    return {
  ...
  methods: {
    ...
    async loadEmployeesJob() {
      // 获取员工岗位信息
      this.formData = await gitEmployeesJobInof(this.userId)
    },
    // 点击保存员工岗位信息
    async saveEmployeesJob() {
      try {
        // 保存员工岗位信息
        await saveEmployeesJobInof(this.formData)
