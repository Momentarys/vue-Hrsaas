    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <!-- 放置上传图片 -->
          <upload-img ref="uploadPic" :default-url="employeesPic" @on-success="uploadPicSuccess" />
        </el-form-item>
      ...
      <div class="block">
        ...
        <el-row class="inline-info" type="flex" justify="center">
          <el-col :span="12">
            <el-button type="primary" @click="saveEmployeesInof">保存更新</el-button>
            <el-button @click="$router.back()">返回</el-button>
          </el-col>
        </el-row>
      </div>
...
import { getEmployeesInof, saveEmployeesInof } from '@/api/employees'
export default {
  data() {
    return {
  ...
  methods: {
    ...
    // 获取员工个人信息
    async loadEmployees() {
      const res = await getEmployeesInof(this.userId)
      // 如果有图片就把链接赋值给变量
      if (res.staffPhoto) {
    ...
    async saveEmployeesInof() {
      try {
        ...
        // 调用保存员工个人信息
        await saveEmployeesInof(this.formData)
